...
- **Expecting prefix caching to change outputs.** It reuses byte-identical KV for a shared prefix — the result is unchanged. If you see different outputs with caching on, that's a bug, not the feature. (Details are the [next lesson](index.md).)
