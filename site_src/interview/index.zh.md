...
> 一个**精选**的**高频**面试题库，按模块（Part 0、2–8）归类。每题遵循同一 schema：**直接答案 → 深入原理 → 代码（若适用）→ 面试官追问 → 关联知识点**。
...
- **Part 5 · 服务化与吞吐（vLLM 核心）**
    ...
    - [抢占：recompute vs swap](preemption.md) — KV 池耗尽时 vLLM 怎么做、V1 为何默认 recompute（并去掉 swap）、以及阻止它的容量旋钮。
    ...
    - [采样参数：temperature、top-p/top-k 与吞吐](sampling-parameters.md) — 采样旋钮与 greedy 解码、它们为何几乎不动吞吐、以及固定采样为何让 eval 可复现。
- **Part 6 · 进阶推理专题**
...
- **Part 7 · 多卡与分布式**
    ...
    - [MoE 推理：激活参数 vs 总参数与专家路由](moe-inference.md) — 激活 vs 总参数与 FLOPs、router 如何逐 token 挑专家、以及为何专家的显存在多卡上答 EP（而非 TP）。
...
!!! note "如何使用本题库"
    每题都与它考察的课程双向链接——先读课、再刷题，或直接硬刚一道题、需要机制时顺链回看。这是一份精选高频集，而非穷举集；难度档 / 频率标签 / 权重暂不在范围。
