...
- **Setting a TP degree that doesn't divide the heads.** TP splits attention heads, so the degree must evenly divide the head (and KV-head) count — that's why it's a power of 2. An arbitrary degree fails or wastes GPUs.
