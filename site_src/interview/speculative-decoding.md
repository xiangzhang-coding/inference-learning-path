...
**Speedup = expected accepted run length** $\sum_{i=0}^{K}\alpha^i=(1-\alpha^{K+1})/(1-\alpha)$, where α is the per-token draft/target agreement. At α=0.7, K=4 ≈ 2.77× fewer target passes.
...
- **Acceptance rate is everything.** A cheap draft that rarely agrees (low α) wins little; the design tension is a draft that's *both* cheap *and* agrees often (why EAGLE exists).
- **Diminishing K.** $\sum\alpha^i$ saturates; past a point extra draft tokens rarely survive but always cost draft compute. Tune K to α.
...
- *"Bigger K always better?"* → No — diminishing returns via $\sum\alpha^i$; large K on low α wastes draft compute. Match K to α.
...
- *"What's the single biggest lever?"* → Acceptance rate α (draft/target alignment) — it dominates the whole $\sum\alpha^i$.
