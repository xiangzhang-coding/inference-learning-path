...
!!! note "Part 7 完成"
    两节课已落地——**[并行策略](parallelism-strategies.md)**（*为什么与选哪种*）与 **[NCCL + 启动 TP/PP](nccl-and-launching-tp-pp.md)**（多卡上手）——各与其面试题（[并行策略：TP/PP/DP/EP](../interview/parallelism-strategies.md)、[NCCL 集合通信与启动 TP/PP](../interview/nccl-collective-communication.md)）双向链接。压测与并发**拐点（knee）**在 **Part 8** 讲。所有 vLLM flag/API 均按 ADR-0004 用 Context7 核实；基线为 **vLLM 0.26.0**，一切性能数字均为**示例 / 量级参考**。见 **[术语表](../glossary.md)** 与 [面试题库](../interview/index.md)。
