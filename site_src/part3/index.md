...
!!! note "Scaffolding status"
    Part 3 is complete: all four lessons (tickets #8, #9) are written, each with a two-way-linked interview question — the GPU-programming mental models (execution model, memory access) and the hands-on half (Triton kernels, reading the PagedAttention kernel). Hand-written/optimized CUDA C++ stays out of scope (ADR-0002); the PagedAttention *serving* deep-dive (continuous batching, block manager) lands in **Part 5**. See the [Interview Bank](../interview/index.md) for the linked question set.
