...
**RoPE** 通过旋转 Q 和 K 注入位置——**无权重、FLOPs 微不足道**，其外推特性正是长上下文得以成立的基础（→ Part 6）。

**MoE** 把单个 FFN 换成 $E$ 个专家，但每 token 只路由到其中 $k$ 个。参数暴涨（全部 $E$ 个专家都驻留 VRAM），而**每 token 激活 FLOPs 接近一个 dense 的 $k$-专家模型**。所以 MoE 用*权重 VRAM* 换*每 token 廉价算力*——成本地图上的另一个点（→ Part 7）。Qwen2.5-7B 是 **dense**，故下面的计数器是 dense 版。
