...
!!! note "Part 8 完成"
    七节课全部就位——[OpenAI 兼容 server](openai-server.md)、[压测找 knee](load-testing-knee.md)、[路由 / 自动扩缩](routing-autoscaling.md)；[可观测性与 profiling](observability-profiling.md)、[SLO 驱动调优](slo-driven-tuning.md)、[服务生态对比](framework-comparison.md)；以及[容量规划 + 系统设计](capacity-planning.md)——各自与其面试题双向链接，最后一节还连到一组**[系统设计长题](../interview/system-design.md)**。所有 vLLM flag/API 均经 Context7 核实（ADR-0004）；基线是 **vLLM 0.26.0**，一切性能数字均为**示例 / 量级参考**。下一站：**[Capstone](../capstone/index.md)**。见 **[术语表](../glossary.md)** 与 [面试题库](../interview/index.md)。
